...
// .wrangler/tmp/bundle-gIjW92/checked-fetch.js
...
// .wrangler/tmp/pages-uRF3Nw/functionsWorker-0.12673805966017493.mjs
...
var rateLimit = /* @__PURE__ */ __name2(async (c, next) => {
...
api.post("/polls", async (c) => {
  ...
  try {
    const statements = [
      db.prepare("INSERT INTO polls (id, title, description) VALUES (?, ?, ?)").bind(pollId, title, description || null)
    ];
    for (const opt of options) {
      const optId = `o_${nanoid(16)}`;
      statements.push(
        db.prepare("INSERT INTO poll_options (id, poll_id, start_time, end_time) VALUES (?, ?, ?, ?)").bind(optId, pollId, opt.start_time, opt.end_time ?? null)
      );
    }
...
api.get("/polls/:id", async (c) => {
  ...
  try {
    ...
    const rankings = Object.entries(scores).map(([id, score]) => ({ option_id: id, score })).sort((a, b) => b.score - a.score);
    const maxScore = rankings[0]?.score || 0;
    const optimal_option_ids = rankings.filter((r) => r.score === maxScore && maxScore > 0).map((r) => r.option_id);
    return c.json({
      ...poll,
...
api.post("/polls/:id/vote", async (c) => {
  ...
  const editToken = `e_${nanoid(32)}`;
  try {
    const statements = [
      db.prepare("INSERT INTO responses (id, poll_id, voter_name, edit_token) VALUES (?, ?, ?, ?)").bind(responseId, pollId, voter_name, editToken)
    ];
    for (const vote of votes) {
      statements.push(
        db.prepare("INSERT INTO votes (response_id, option_id, status) VALUES (?, ?, ?)").bind(responseId, vote.option_id, vote.status)
      );
    }
    await db.batch(statements);
...
api.get("/polls/:id/response/:editToken", async (c) => {
  ...
  const db = c.env.DB;
  try {
    const res = await db.prepare("SELECT id, voter_name FROM responses WHERE poll_id = ? AND edit_token = ?").bind(pollId, editToken).first();
...
api.put("/polls/:id/response/:editToken", async (c) => {
  ...
  const db = c.env.DB;
  try {
    const response = await db.prepare("SELECT id FROM responses WHERE poll_id = ? AND edit_token = ?").bind(pollId, editToken).first();
    ...
    for (const vote of votes) {
      statements.push(
        db.prepare("INSERT INTO votes (response_id, option_id, status) VALUES (?, ?, ?)").bind(response.id, vote.option_id, vote.status)
      );
    }
    await db.batch(statements);
...
// .wrangler/tmp/bundle-gIjW92/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-gIjW92/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.12673805966017493.js.map
